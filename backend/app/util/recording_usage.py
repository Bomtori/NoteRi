...
def use_minutes(db: Session, user_id: int, seconds: int) -> RecordingUsage:
    """
    ...
    if usage.allocated_seconds is None or (
        isinstance(usage.allocated_seconds, int) and usage.allocated_seconds < 0
    ):
        # 차감 없이 그대로 반환 (필요하면 여기서 활동 로그만 남기면 됨)
        return usage

    # 유한 플랜 차감
    new_used = int(usage.used_seconds or 0) + int(seconds)
    alloc = int(usage.allocated_seconds or 0)

    if new_used > alloc:
        remaining = max(alloc - int(usage.used_seconds or 0), 0)
        raise UsageExceededError(f"Recording seconds exceeded. remaining={remaining}")
