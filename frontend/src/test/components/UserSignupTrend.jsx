...
import DateButtons from "@/test/components/DateButtons"               // 하나만 import (중복 X)
...
export default function UserSignupTrend() {
  ...
  // 응답 → {x,y}[] 정규화
  const normalizeToXY = (resp) => {
    const rows = resp?.data ?? resp?.items ?? resp ?? []
    return rows.map(d => ({
      x: d.x ?? d.date ?? d.day ?? d.week ?? d.month ?? d.label ?? "",
      y: Number(d.y ?? d.count ?? d.value ?? d.total ?? 0),
    }))
  }
