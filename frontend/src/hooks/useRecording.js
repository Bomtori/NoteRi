...
//       const targetBoardId = id || boardId; // ✅ 명시적 전달 or fallback
//       if (!targetBoardId) throw new Error("boardId is required to start recording");
//     if (recordingState === "idle") return;
//     setRecordingState("idle");
//     pausedRef.current = false;
//
...
export default function useRecording({ WS_URL, onData, onStartError }) {
    ...
    // ✅ boardId를 인자로 받도록 수정
    const startRecording = async (boardId) => {
        if (recordingState !== "idle") return;
        ...
        try {
            setRecordingState("connecting");
            pausedRef.current = false;
            currentBoardIdRef.current = boardId; // ✅ ref에 저장
            ...
            await new Promise((resolve, reject) => {
                ...
                wsRef.current.onclose = (e) => {
                    ...
                    if (!resolved) {
                        clearTimeout(timeout);
                        reject(new Error(`WS closed (${e.code}) ${e.reason || ""}`));
                    }
                };
